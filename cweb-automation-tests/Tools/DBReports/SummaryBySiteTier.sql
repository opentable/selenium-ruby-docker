...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[SummaryBySiteTier]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[SummaryBySiteTier]
GO

create procedure dbo.SummaryBySiteTier
AS
	...
	and			dlm.Tier			= r.Tier
	group by	r.SiteType, r.Tier
	order by	r.SiteType desc, r.Tier asc
GO

GRANT EXECUTE ON [SummaryBySiteTier] TO public
GO
